...
export class HttpClient extends Config {
    ...
    public _AngularRequestGet(url: string, params: string, headers) {
        if(this.mDebugEnable){
            console.log("Angular Request : "+ url+"?"+params);
        }
    ...
    public _NativeRequestGet(url: string, params, headers) {
        if(this.mDebugEnable){
            console.log("Native Request : "+ url);
            console.log("Params : "+ JSON.stringify(params));
        }
        return new Promise((success, fail) => {
            this.mNativeHttp.get(url, params, headers).then(data => {   
                if (data.status == 200) success(JSON.parse(data.data)); else fail(data); }, error => { fail(error); });
        });
    }
